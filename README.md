...
2. **`X-VIN` 里到底该放什么** —— 依次用 `加密(VIN)`、`加密(userVehId)`、
   `userVehId`、`加密(temId)`、明文 `VIN` 去请求同一个接口，记录网关对每个候选
   的应答。**如果某个候选被接受，集成会立即改用它**（`vehicle_token_source`
   会写成 `probe:...`），所以有时**导一次诊断就能把 `X-VIN` 修好**。
