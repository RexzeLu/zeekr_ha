...
车辆列表里每辆车都带 `isOwner` 字段。如果不是车主，也不是被授予控制权的共享
账号，网关就会拒绝对该车的状态与控制接口（表现为 `079001`），而只读的车辆列表
仍然正常。诊断里的 `vehicle_list_raw[].entry.isOwner` 能看到这个值。

> 处理：在「设置 → 设备与服务 → 极氪」中改用**车主手机号**重新配置。若要用其他
> 账号，请先在官方 App 里把车**共享**给它，并确认共享时勾选了控制权限。
