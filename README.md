...
### 实时状态 / 远程控制不生效：需要「车辆 X-VIN 令牌」

**现象**：车况数据都正常（来自 GW2），但 GW3 的**状态**和**指令**接口一律回
`079001 [SDK]此接口未被授权，无法访问!`；指令发出去没有报错，车也不动。

**原因**：新平台（SNCTSP）**不是用 VIN 明文寻址车辆**，而是用 `X-VIN` 头里的
**每车不透明令牌**。它有两个作用：

- **寻址** —— 告诉网关你操作的是哪辆车；
- **授权** —— 令牌本身携带该车的能力（capability），所以它是个凭据，不只是编号。

这个令牌**在 App 内部生成，无法本地算出来**。用自己加密的 VIN 代替是行不通的：
登录接口会接受它（所以 GW3 登录、车辆列表都能成功），但任何需要「车辆能力」的
接口都会判定未授权 —— 这正是 `079001` 的来历。

> 对比：`079025 Decrypt X-VIN failed` 是另一回事，指发出去的 `X-VIN`
> 根本不是密文（比如直接发了明文 VIN）。两者含义不同。

**取法（一次性，之后长期有效）**：从官方 App 的请求里把这一个头的值抄出来。

1. 用 **HTTP Toolkit**（或任意能解密 HTTPS 的抓包工具）让手机走它的代理，
   安装并信任它的证书；
2. 在 App 里**做一次车辆操作**（开一次空调即可）；
3. 在抓到的请求里找 `snc-tsp-api.zeekrlife.com` 的任意一条，**复制
   `X-VIN` 请求头的完整值**（一串不透明的字符串）；
4. 回到 HA：**设置 → 设备与服务 → 极氪 → 配置**，把值粘进
   **「车辆 X-VIN 令牌（可选，取自 App 抓包）」**，保存。

保存后集成会重启该配置项，随后 GW3 的状态与指令接口应当放行。诊断里的
`gateways.vehicle_token_configured` 会显示是否已生效（**令牌本身不会出现在
诊断文件中**）。

> 留空 = 继续用本地加密的 VIN。此时状态会回退到 GW2（数据是缓存快照，
> 不如 GW3 的 `target=new` 新鲜），指令则只有 GW2 那条不太可靠的通道。
